...
export default function AdminUsersPage() {
  ...
  const handleApproveUser = async (userId: string, role: string = 'submitter') => {
    setIsUpdating(true)
    try {
      ...
      if (response.ok) {
        setUsers(users.map(user => 
          user.id === userId ? { ...user, status: 'approved', is_active: true, role: role as any } : user
        ))
      } else {
  ...
  const handleRejectUser = async (userId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        setUsers(users.map(user => 
          user.id === userId ? { ...user, status: 'rejected', is_active: false } : user
        ))
      } else {
                          ...
                          {editingUser?.id === user.id ? (
                            ...
                            <div className="flex space-x-2">
                              {user.status === 'pending' ? (
                                <>
                                  <button
                                    onClick={() => handleApproveUser(user.id, 'submitter')}
                                  ...
                                  <button
                                    onClick={() => handleApproveUser(user.id, 'reviewer')}
