...
export default function SubmissionsPage() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      try {
        const response = await fetch('/api/list', { credentials: 'include' });
        if (response.ok) {
      ...
      // 2. Data now comes only from database - no localStorage fallback
  ...
  const restoreRecord = async (item: Row) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Force immediate UI update by removing from current tab
        setItems(prev => prev.filter(currentItem => currentItem.id !== item.id));
        
        // Update allItems to reflect the status change
        setAllItems(prev => prev.map(currentItem => 
          currentItem.id === item.id 
            ? { ...currentItem, status: 'draft', restoredBy: currentUser, restoredAt: new Date().toISOString() }
            : currentItem
        ));
        ...
        setTimeout(() => {
          loadItems();
          setActiveTab('draft'); // Auto-switch to draft tab
        }, 200);
      } else {
  ...
  const deleteRecord = async (item: Row) => {
    console.log('🗑️ Delete button clicked for:', item.id, item.savedAt);
    ...
    try {
      ...
      if (result.ok) {
        ...
        setAllItems(prev => prev.map(currentItem => 
          currentItem.id === item.id 
            ? { ...currentItem, status: 'deleted', deletedBy: currentUser, deletedAt: new Date().toISOString() }
            : currentItem
        ));
        ...
        setTimeout(() => {
          loadItems();
          setActiveTab("deleted"); // Auto-switch to deleted tab
        }, 200);
      } else {
      ...
      setMessage({ 
        type: 'error', 
        text: `Failed to delete record: ${error instanceof Error ? error.message : 'Unknown error'}` 
      });
    } finally {
      setProcessingId(null);
      setTimeout(() => setMessage(null), 5000);
    ...
    try {
      ...
      if (result.ok) {
        ...
        setAllItems(prev => prev.map(item => 
          item.id === id 
            ? { ...item, status: 'submitted', submitterMessage: note.trim() }
            : item
        ));
  ...
  try {
    ...
    return (
      <div className="p-4">
        <h1 className="text-2xl font-semibold text-red-600">Error Loading Submissions</h1>
        <p className="text-gray-600 mt-2">There was an error loading the submissions page. Please refresh the page.</p>
        <button 
